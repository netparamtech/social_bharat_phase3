        ...
        <div id="searchPeople-section" className="content-wrapper pt-4 mb-4">
            ...
            <div className="container">
                <div className="card shadow card-search">
                    <div className="card-header mx-auto mt-2 fs-3 fw-bold">ACTIVITIES</div>

                    <div className="d-flex justify-content-between">
                        <div className="m-2 mt-2 col-6 col-sm-6">
                        ...
                        <button className="btn bg-darkskyblue btn-success btn-sm small hover-pointer btn-post-activity" onClick={()=>navigate('/user/post-activity')}>Post Your Activity</button>
                    </div>
                                            ...
                                            pair.map((item, idx) => (
                                                <div className="col-md-6 col-sm-6 col-12" key={idx}>
                                                    ...
                                                    <div className="card mb-3 position-relative">
                                                        <div className="card-body  bg-darkskyblue">
                                                            <Card className=" bg-darkskyblue"
                                                                ...
                                                                <div className="d-flex justify-content-between">
                                                                    <Meta
                                                                        avatar={item.user_profile ? (
                                                                            ...
                                                                            <button type='button' className='dropdown-user-img-letter m-2'>{user.user.name.charAt(0).toUpperCase()}</button>
                                                                        )}
                                                                        title={item.name}
                                                                        description={calculateTimeDifference(item.updated_at)}
                                                                    />

                                                                    <NavDropdown title={customTitle} id="basic-nav-dropdown" disabled={user.user.id === item.user_id ? false : true}>
                                                                        <NavDropdown.Item className="hover-pointer" onClick={() => navigate(`/user/activity/update/${item.id}`)} disabled={user.user.id === item.user_id ? false : true}>Edit Activity</NavDropdown.Item>
                                                                        <NavDropdown.Item className="hover-pointer" onClick={() => deleteSingleActivity(item.id, item.user_id)} disabled={user.user.id === item.user_id ? false : true}>Delete Activity</NavDropdown.Item>
                                                                        <NavDropdown.Divider />
                                                                        <NavDropdown.Item href="#">Manage Your Activities</NavDropdown.Item>
                                                                    </NavDropdown>
                                                                </div>

                                                                {item.photo && Array.isArray(item.photo) && item.photo.length > 0 ? (
                                                                    <div className="photo-container mt-2 dotted-border-carousel">
                                                                        <Carousel className="photo-carousel" dotPosition="bottom" autoplay effect="fade">
                                                                    ...
                                                                    item.photo ? (
                                                                        ...
                                                                        />
                                                                    ) : <p className="fs-4 m-2 text-light">{item.DESCRIPTION}</p>}
