...
import { currentJobById, fetchAllCitiesByStateID, fetchAllStatesByCountryID, updateCurrentJob } from '../../services/userService';
...
const UpdateCurrentJob = () => {
    ...
    const fetchJob = async () => {
        dispatch(setLoader(true));
        try {
            const response = await currentJobById(id);
            if (response && response.status === 200) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await updateCurrentJob(updatedData,id);

            if (response && response.status === 201) {
                setErrors('');
                setServerError('');
                toast.success("Successfully Updated Job", successOptions);
                dispatch(setLoader(false));
            }
        } catch (error) {
            dispatch(setLoader(false));
                            ...
                            <div className="col-md-12 col-sm-12 col-xs-12 p-4">
                                <form onSubmit={handleSubmit} className="w-100 w-lg-75">
                                    <fieldset className="shadow">
                                        <legend className='d-flex justify-content-between'>Update Current Job Opening
